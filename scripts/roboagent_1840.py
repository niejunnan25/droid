...
class RoboAgent:
                ...
                if is_post_run == False:
                    ...
                    if current_subtask_local is not None:
                        ...
                        with timer.timer("subtask complete check", self.log_file):
                            subtask_completed_str : str = self.planner.request_task(
    ...
    def get_global_instruction(self) -> list:
        ...
        # self.initial_image 和 self.user_prompt 不是共享变量
        
        with timer.timer("global instruction proposal", self.log_file):
            # TODO: planer 类
            global_task_str : str = self.planner.request_task(
    ...
    def check_global_complete(self) -> bool:
        ...
        with timer.timer("global task complete check", self.log_file):
            ...
            task_completed_str : bool = self.planner.request_task(
                task_name="prompt_completion_check",
                vl_inputs={
                    "images": {
                        "initial_image": self.initial_image,
    ...
    def run_agent(self):
        ...
        while True: # Outer loop (for entire task)
            ...
            # 运行任务之前, 先检查当前任务是否已经完, 如果已经完成则 close_episode
            # if self.check_complete():
            #     self.close_episode()
            #     break # Task was already complete

            # TODO: Complete the get_global_instruction func
            ...
            while True: # Inner loop (for sub-tasks)
                ...
                with self.current_subtask_lock:
                    try:
                        self.current_subtask : str = subtask_list[subtask_index]
                        current_subtask : str = self.current_subtask
                    except Exception as e:
                        self.log("Current subtask list out of index", "error")
